...
creates `dw`, `redacted_db_1` and `redacted_db_2`. The read-only user could not
...
addresses two of them by a prefixed name (`redacted_db_2`, `redacted_db_1`),
...
| `redacted_db_2` | 109 / 110 | 43 / 43 pass |
| `redacted_db_1` | 175 / 175 | 30 / 30 pass |
...
The remaining 15 questions reference `keystone` (8), `redacted_db_3` (5), and
`redacted_db_4` (2), for which **no dumps are published upstream**. They fail with a
