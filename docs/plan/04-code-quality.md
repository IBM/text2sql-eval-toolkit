...
### 4.8 Version hygiene → the 1.3.0 release — **done, at a different number**
This item said 2.0.0, on the reasoning that "the URL scheme, capability tiers, artifact
index, and deployment model are all new or breaking". That was written before the work
existed. Checked against the finished branch, none of it breaks a documented consumer:

| Surface | Result |
|---|---|
| Curated public API (`__init__.__all__`, 41 names) | no removals, no additions |
| CLI subcommands | identical |
| Artifact JSON contract | unchanged — a stated principle of this plan |
| Local dashboard behaviour | unchanged; tiers default to `full` on loopback, with a non-regression test |
| `evaluate_prediction` | its new `raise` is caught by the existing handler and surfaces as `eval_error` — a fix, not a contract change |

The URL scheme is *new* rather than breaking: there were no URLs before, so no links
broke. `ui/server.py` lost ~85 module-level names to the router split, but the project's
own contract (CLAUDE.md) is that the public API is curated in `__init__.py`.

So **1.3.0**. It also follows 1.2.0 naturally, so the changelog skew resolves by moving
past it, and it stays inside the published snapshot's `>=1.1.0,<2.0.0`, which means
`results fetch` keeps working with no re-upload and no loosening of a check under release
pressure.

Done:

1. `pyproject.toml` is the single source; `_version.py` reads it back from the installed
   distribution metadata, and both `__init__` and `results/_hub` import from there. The
   number was previously written in three places, which is how the package came to claim
   1.1.0 while the changelog documented 1.2.0.
2. CHANGELOG entry for 1.3.0.
3. `scripts/ci/check_version.py`, run in CI: pyproject, the installed package, the
   changelog and the git tag must agree.

Deliberately **not** done, and why:

- **No git tag, no push.** A tag pointing at a commit that may still change is worse than
  no tag. Both wait for the pre-push review.
- **No `v1.3.0` Hub snapshot.** `DEFAULT_REVISION` is derived as `v{version}`, so 1.3.0
  asks for a `v1.3.0` tag, does not find one, warns once and falls back to `main`.
  Non-fatal; pinned links are not reproducible until a snapshot is tagged. Needs `HF_TOKEN`
  with write access, so it is the maintainer's step, not a release blocker.
- **The `schema_version` flaw is untouched.** The manifest carries `schema_version: 1` —
  the actual data-format contract — and nothing ever reads its value, while the enforced
  gate is a toolkit-version range generated by assumption in the upload script. 1.3.0
  steps around that trap rather than fixing it, and it will be waiting for whenever a
  2.0.0 happens. Worth fixing; not worth bundling into a version commit.
