...
def main():
    ...
    for benchmark_id, benchmark_info in benchmarks.items():
        try:
            ...
            data_path = Path(benchmark_info["benchmark_json_path"])
            ...
            if dialect == "db2":
                dialect = "postgres"
            profile_pred_or_eval_json_file(str(data_path), dialect)
            if predictions_path.exists():
                profile_pred_or_eval_json_file(str(predictions_path), dialect)
            if eval_path.exists():
                profile_pred_or_eval_json_file(str(eval_path), dialect)
