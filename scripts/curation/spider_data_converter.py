...
def convert_spider_to_toolkit_format(
    ...
    if len(spider_data) != len(gold_queries):
        print(
            f"⚠️  Warning: spider_data has {len(spider_data)} entries but gold_queries has {len(gold_queries)} entries"
        )
    ...
    if sample_size and sample_size < len(indices):
        ...
        print(
            f"✂️  Randomly sampled {sample_size} entries from {len(spider_data)} total (seed: {random_seed})"
        )
...
def main():
    ...
    parser.add_argument(
        "--input", required=True, help="Input Spider JSON file (e.g., spider-dev.json)"
    )
    parser.add_argument(
        "--gold", required=True, help="Gold SQL file (e.g., spider-dev_gold.sql)"
    )
