...
Recommended small models for CPU testing:
  ...
  - microsoft/phi-2 (2.7B parameters)
    ...
    if not check_port_available(args.port):
        ...
        print(
            "\033[1;33m  python scripts/inference/vllm/start_vllm_local.py --port 8001\033[0m"
        )
        print()
        sys.exit(1)
    ...
    print()
    print(
        f"\033[0;32mAPI will be available at: http://{args.host}:{args.port}/v1\033[0m"
    )
    print()
    print(
        "\033[1;33mNote: First run will download the model (may take a few minutes)\033[0m"
    )
    ...
    if not use_gpu:
        ...
        cmd.extend(["--dtype", "float32", "--enforce-eager"])
